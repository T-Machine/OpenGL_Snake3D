...
void SnakeObject::DrawModel(SpriteRenderer &renderer, Model model_head, Model model_body, int frame_count) {
	for (int i = 0; i < nodes.size(); i++) {
		...
		float move_x = x + (x1 - x) / 5 * frame_count;
		float move_z = z + (z1 - z) / 5 * frame_count;
		...
		float rotate_offset = rotate4[d] + temp / 5 * frame_count;

		if (i == 0) {
			renderer.DrawSpriteModel(model_head, glm::vec3(move_x, -0.2, move_z), glm::vec3(1.0f), rotate_offset, glm::vec3(0.0f, 1.0f, 0.0f));
			//renderer.DrawSpriteModel(model_head, glm::vec3(move_x, 0, move_z), glm::vec3(0.03f), rotate_offset, glm::vec3(0.0f, 1.0f, 0.0f)); //car
		}
		else {
			renderer.DrawSpriteModel(model_body, glm::vec3(move_x, 1.0, move_z), glm::vec3(5.0f), rotate_offset, glm::vec3(0.0f, 1.0f, 0.0f));
		}
	}
}